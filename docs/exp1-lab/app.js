// ============================================================================
// EXP1 人工盲測驗證 — 實驗室/資訊背景版 (exp1-lab)
// 分工設計：
//   本表單負責 4 個維度（正確性、推理一致性、可操作性、完整性），
//   農業深度改由農業專家在獨立表單（/exp1-agri/）評分，
//   因為資訊背景評測者難以可靠判斷農業深度，農業專家則難以理解
//   AI評測用語（幻覺、可操作性等抽象概念），故依專業分工評分。
//   本表單額外的「整體判斷」是主要產出，用來跟 Claude Opus(EXP1裁判)
//   的verdict計算Cohen's κ，驗證LLM-as-Judge的可靠性。
// 沿用 /q2-sop-v2/ 的桌面懸浮版面、生育階段對照表、截斷警示、GT顯示。
// cleanText() 已更新：
//   1. 條列/數字列表規則放寬，處理 Gemma4/Gemini 常見的縮排巢狀條列
//   2. 新增「統一每句斷行」規則，讓 ft_qwen 慣常的整段式文章跟其他
//      模型慣用的多行/條列式呈現，在斷行密度上更一致，避免評測者
//      單純因為「看起來比較有條理」而偏向某一邊
// ============================================================================
...
const STORAGE_KEY = "exp1_lab_progress_v1";

// 四個維度 × (A, B) 的分數欄位（不含農業深度）
const SCORE_FIELDS = [
  "criterion_1_accuracy_a", "criterion_1_accuracy_b",
  "criterion_2_reasoning_a", "criterion_2_reasoning_b",
  "criterion_3_actionability_a", "criterion_3_actionability_b",
  "criterion_4_completeness_a", "criterion_4_completeness_b",
];
...
// 去除 Markdown 排版，並統一每句斷行，讓四個模型的呈現方式盡量一致
function cleanText(text) {
  ...
  return text
    ...
    .replace(/\*(.+?)\*/g, "$1")               // *斜體*
    .replace(/#{1,6}\s*/g, "")                 // ## 標題
    .replace(/^\s*[-*+]\s+/gm, "")            // 條列 bullet（含縮排巢狀，Gemma4/Gemini常見）
    .replace(/^\s*\d+\.\s+/gm, "")            // 數字條列（含縮排巢狀）
    .replace(/[✅⚠️📌💡📸🌾]/gu, "")          // 常見 emoji
    .replace(/^---+\s*$/gm, "")              // 水平線
    .replace(/([。！？])(?!\n)/g, "$1\n")     // 統一每句斷行（四個模型都套用，避免斷行密度差異造成偏誤）
    .replace(/\n{3,}/g, "\n\n")              // 連續空行壓縮
    .trim();
}
...
function downloadBackup() {
  ...
  a.href = url;
  a.download = `exp1_lab_${safeName}_backup_${ts}.json`;
...
function appsScriptJsonpRequest(params = {}, timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    const callbackName = `exp1labEndpointPing_${Date.now()}_${Math.random()
      .toString(36)
      .slice(2)}`;
...
async function onSubmit() {
  ...
  const safeName = (evaluatorName || "unknown").replace(/[^\w一-鿿-]/g, "_");
  const ts = new Date().toISOString().replace(/[:.]/g, "-");
  const submissionId = `exp1_lab_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  const wrapped = {
    submissionId,
    fileName: `exp1_lab_${safeName}_${ts}.json`,
    content: payload,
  };
