// ============================================================================
// Q2 盲測評測 — SOP 評分版 (exp3-q2-sop)
// 與 /q2/ 版本差異：
//   1. 五個維度改為「行為錨定評分量表」(BARS)：A、B 各自獨立評 0~3 分，
//      不再是「A較好/B較好/平手」的比較式判斷，勝負由後續分析階段
//      比較 A、B 分數計算得出，藉此降低月暈效應對評分的干擾
//   2. STORAGE_KEY / app_version / fileName 前綴均換為 q2sop 識別字
//   3. 每題分數 0（如 "0"）必須視為「已作答」，不可用 truthy 判斷
//      （因為字串 "0" 雖然是 truthy，但仍要小心分數 0 與未作答 null 的區分）
//   4. 其餘（cleanText、GT 顯示、截斷警示、Apps Script 傳送）與 /q2/ 相同
// ============================================================================
...
const STORAGE_KEY = "exp3_q2sop_progress_v1";
...
function downloadBackup() {
  ...
  a.href = url;
  a.download = `exp3_q2sop_${safeName}_backup_${ts}.json`;
...
async function onSubmit() {
  ...
  const safeName = (evaluatorName || "unknown").replace(/[^\w一-鿿-]/g, "_");
  const ts = new Date().toISOString().replace(/[:.]/g, "-");
  const submissionId = `exp3_q2sop_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  const wrapped = {
    submissionId,
    fileName: `exp3_q2sop_${safeName}_${ts}.json`,
    content: payload,
  };
